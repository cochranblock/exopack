...
**Why:** The crate had drifted from documentation. The `ats_fixtures` module is the largest contract surface added since v0.3 — downstream test benches in `kova` and `atsisbroken` depend on stable vendor-key mappings, and any silent drift in `expected_keys` corrupts every consumer's classifier asserts. `harvest` had no Err-path test at all, meaning a refactor that turned a `Result::Err` into a panic could ship undetected. "Add new tests, replace older ones" — interpreted here as augmenting consumer-boundary coverage for the two largest gaps, not retiring any extant tests (none were observably stale).
**Commits:** (this commit)
**AI Role:** AI mapped the public surface of both modules via `grep`, drafted both integration-test files, iterated on the canonical vocab list after the first run uncovered the Ashby `address`-not-`street1` quirk, and refreshed the PoA metrics table. Human directed: scope ("largest gaps first"), interpretive call on "replace older ones" (no observable older tests to retire), and the doctrine that test files state *why* each invariant matters (so a future reader knows whether to delete or update the test when behavior changes).
