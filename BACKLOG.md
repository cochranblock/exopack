...
- [x] `ats_fixtures` feature pulled in from kova: 5-vendor mock ATS HTML generator (Greenhouse, Lever, Workday, iCIMS, Ashby) + adversarial knobs (late hydration, dynamic IDs, rebuild-on-focus); `exopack ats-fixture <vendor>` CLI subcommand
- [x] Fixed pre-existing buggy assertion in `expected_keys_cover_full_vocab_for_workday` test (asserted "full_name" for a split-name vendor)
...
16. [refactor] Switch `kova/src/exopack/ats_fixtures.rs` to re-export from this crate (`pub use exopack::ats_fixtures::*;`) so atsisbroken automatically picks up the canonical version + the bug-fixed test. Single source of truth.
17. [feature] Add iCIMS-classic and Greenhouse-Boards fixture variants once a second consumer documents the selectors
